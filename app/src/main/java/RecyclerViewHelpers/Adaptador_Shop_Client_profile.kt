...
class Adaptador_Shop_Client_profile( var Datos: List<DataInventory>, private val sharedViewModel: SharedViewModel_Product_Client, private val ShareViewModel: ImageViewModel_Client
) : RecyclerView.Adapter<ViewHolder_Shop_Client_Profile>() {
    private var datosFiltrados = Datos

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder_Shop_Client_Profile {
        val vista = LayoutInflater.from(parent.context).inflate(R.layout.activity_card_shop_client, parent, false)
        return ViewHolder_Shop_Client_Profile(vista)
    }

    override fun getItemCount() = datosFiltrados.size

    override fun onBindViewHolder(holder: ViewHolder_Shop_Client_Profile, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            ...
            navController.navigate(R.id.navigation_Details_favorites_Profile, bundle)
        }
    }
