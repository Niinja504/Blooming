package proyecto.expotecnica.blooming.Employed.notifications

import DataC.Data_Notifications
import RecyclerViewHelpers.Adaptador_Notifications_Admin
import RecyclerViewHelpers.Adaptador_Notifications_Employed
...
class Notifications : Fragment() {
    ...
    private var miAdaptador: Adaptador_Notifications_Employed? = null
    ...
    override fun onCreateView(
        ...
        val root = inflater.inflate(R.layout.fragment_notifications_employed, container, false)

        val IC_Regresar = root.findViewById<ImageView>(R.id.IC_Regresar_Notifications_Employed)
        val IMGUser = root.findViewById<ImageView>(R.id.IMG_User_Notifications)
        val RCV_Notifications = root.findViewById<RecyclerView>(R.id.RCV_Notifications_Employed)
        //Asignarle un Layout al RecyclerView
        RCV_Notifications.layoutManager = LinearLayoutManager(requireContext())
        Buscador = root.findViewById(R.id.txt_Buscar_Notifications_Employed)
        val LimpiarBuscador = root.findViewById<ImageView>(R.id.IC_Limpiar_Bucador_Notifications_Employed)
            ...
            override fun run() {
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    withContext(Dispatchers.Main) {
                        miAdaptador = Adaptador_Notifications_Employed(NotificationsDB)
