...
class Adaptador_Notifications_Client (var Datos: List<Data_Notifications>): RecyclerView.Adapter<ViewHolder_Notifications_Client>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder_Notifications_Client {
        //Conectar el RecyclerView con la Card
        val vista = LayoutInflater.from(parent.context).inflate(R.layout.activity_card_notifications_client, parent, false)
        return ViewHolder_Notifications_Client(vista)
    }

    override fun getItemCount() = datosFiltrados.size

    override fun onBindViewHolder(holder: ViewHolder_Notifications_Client, position: Int) {
